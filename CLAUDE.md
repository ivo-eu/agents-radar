...
export PUBLISH_ISSUES=false      # set true only when issue publishing is wanted
...
- Search: generated monthly indexes in `search-index/` are loaded only after the user searches.
- Local sync: `sync-manifest.json` contains every digest path, size and SHA-256 for Radar Bridge.
- Favorites: the Pages UI talks only to the paired loopback Bridge at `127.0.0.1:4399`.
- RSS Feed: `feed.xml` at the repo root. Generated by `src/generate-manifest.ts` in the same `pnpm manifest` step. Contains the latest 30 items (newest first) across all report types. Item links use hash routing: `https://ivo-eu.github.io/agents-radar/#YYYY-MM-DD/report`.
