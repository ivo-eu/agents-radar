...
> ② SecondBrain vault 的 `日报/AI情报/收藏/AI情报收藏.md`（Obsidian 读）—— 一次同步两份产物一起写。
...
- **删除运行路径**：旧 `renderFavorite`、`handleFavorite`、`handleFavoriteCheck`。
- `FavoriteCandidate`、`favoriteID`、`normalizeFavoriteURL` 仅保留给浏览器/Go ID 契约测试，不参与实时同步。
- 删除 `State.Favorites map[string]string`（旧 id→vault 路径）；旧 `state.json` 中该字段会被 Go JSON 解码安全忽略。
...
7. 渲染并写 VaultPath/日报/AI情报/收藏/AI情报收藏.md（§3.6，跳过墓碑）
...
写 `VaultPath/日报/AI情报/收藏/AI情报收藏.md`，跳过墓碑，按收藏日期（`created_at` 当地日期）倒序分组，组内时间倒序：
...
- 文件头注明“自动生成勿手改”（避免被覆盖）。
- 旧的单条 `收藏/YYYY/YYYY-MM/*.md` 已在 Phase 2 验收后删除，不再新增。
...
- ✅ 不读 vault 其他内容、不上传 vault 内容（只写 `日报/AI情报/收藏/AI情报收藏.md`）。
